...
/**
 * HT-002.4.1: Cross-browser Testing Script
 * 
...
function testBrowserAvailability(browser) {
  try {
    const version = execSync(browser.command, { encoding: 'utf8', timeout: 5000 });
    return {
      available: true,
    ...
    if (browser.fallback) {
      try {
        const version = execSync(browser.fallback, { encoding: 'utf8', timeout: 5000 });
        return {
          available: true,
          version: version.trim(),
...
async function runCrossBrowserTests() {
  ...
  for (const browser of browserTests) {
    ...
    if (result.available) {
      console.log(`  ✅ ${browser.name} available: ${result.version}`);
