...
class HeroUnifiedOrchestrator {
  constructor() {
    ...
    // UNIFIED: Retry helper for resilient operations
    ...
    // UNIFIED: Concurrency limiters for bounded resource management
    this.concurrencyLimiters = {
      ...
      buildOperations: new ConcurrencyLimiter({
        maxConcurrent: 3,
        maxQueueSize: 20,
        priorityLevels: 5,
        timeoutMs: 300000, // 5 minutes for builds
        resourceLimits: { cpu: 70, memory: 80, disk: 60 }
      }),
      testOperations: new ConcurrencyLimiter({
        maxConcurrent: 5,
        maxQueueSize: 30,
        priorityLevels: 4,
        timeoutMs: 60000, // 1 minute for tests
        resourceLimits: { cpu: 75, memory: 85, disk: 65 }
      }),
  ...
  // STEP 4: VERIFY - Comprehensive health check with bounded concurrency
  async performComprehensiveHealthCheck() {
    ...
    try {
      ...
      const healthCheckOperations = Object.entries(UNIFIED_SYSTEM_REGISTRY).map(([key, system], index) => ({
        operation: async () => {
          try {
        ...
        priority: index < 5 ? 1 : 2, // First 5 systems get priority 1
        metadata: { system: key, type: 'healthCheck' }
      }));
      
      // Execute health checks with bounded concurrency
      await this.concurrencyLimiters.healthChecks.executeBatch(
      ...
      return await this.performFallbackHealthChecks();
    }
  }
  ...
  // Check system health with retry logic
  async checkSystemHealth(key, system) {
    try {
      ...
      // Use retry helper for resilient health checks
      ...
      if (!result.success) {
        console.warn(`⚠️ Health check failed for ${key} after ${result.attempts} attempts`);
        return { score: 0.3, status: 'unhealthy', error: result.lastError?.message || 'Health check failed' };
      }
      
      return { score: 1.0, status: 'healthy', message: 'Health check passed' };
      
    } catch (error) {
      console.error(`💥 Unexpected error in health check for ${key}:`, error.message);
  ...
  async runCommand(command, options = {}) {
    ...
    const { execWithTimeout } = require('./timeout-wrapper');
    ...
    const result = await this.retryHelper.retry(
      async () => {
        try {
          ...
          // Use timeout wrapper with proper error handling
          const execResult = await execWithTimeout(fullCommand, { 
            encoding: 'utf8',
          ...
          if (execResult.success) {
            ...
            throw new Error(`Command failed with exit code ${execResult.exitCode}: ${execResult.stderr}`);
          }
        } catch (error) {
          if (error.message.includes('timeout')) {
            console.warn(`⏰ Command timeout: ${command}`);
            throw new Error(`Command timeout after ${timeout}ms: ${command}`);
          }
          throw error;
        }
      },
      `run-command-${command.replace(/[^a-zA-Z0-9]/g, '-')}`,
      {
        maxAttempts: 3,
        baseDelay: 1000,
        strategy: 'EXPONENTIAL'
    ...
    if (!result.success) {
      console.error(`💥 Command failed after retries: ${command}`);
      console.error(`Last error: ${result.lastError?.message}`);
      throw new Error(`Command failed after ${result.lastError?.message}`);
    }
  ...
  displayUnifiedDashboard() {
    ...
    // Display retry helper statistics
    const retryStats = this.retryHelper.getStats();
    const circuitBreakers = this.retryHelper.getCircuitBreakerStatus();
    
    if (Object.keys(retryStats).length > 0) {
      console.log('\n🔄 Retry Helper Statistics:');
      for (const [operation, stats] of Object.entries(retryStats)) {
        const successRate = stats.success + stats.failure > 0 
          ? ((stats.success / (stats.success + stats.failure)) * 100).toFixed(1)
          : '0.0';
        console.log(`  ${operation}: ${successRate}% success (${stats.success}/${stats.success + stats.failure})`);
      }
    }
    
    if (Object.keys(circuitBreakers).length > 0) {
      console.log('\n🛡️ Circuit Breaker Status:');
      for (const [operation, breaker] of Object.entries(circuitBreakers)) {
        const status = breaker.state === 'OPEN' ? '🚨 OPEN' : 
                     breaker.state === 'HALF_OPEN' ? '🔄 HALF-OPEN' : '✅ CLOSED';
        console.log(`  ${operation}: ${status} (${breaker.failureCount} failures)`);
      }
    }
  ...
  getStatus() {
    return {
      ...
      concurrencyStatus: {
        ...
        buildOperations: this.concurrencyLimiters.buildOperations.getStatus(),
        testOperations: this.concurrencyLimiters.testOperations.getStatus(),
      ...
      concurrencyMetrics: {
        ...
        buildOperations: this.concurrencyLimiters.buildOperations.getMetrics(),
        testOperations: this.concurrencyLimiters.testOperations.getMetrics(),
...
async function main() {
  ...
  try {
    ...
    switch (command) {
      ...
      case 'concurrency':
        const status = unifiedHero.getStatus();
        console.log('📊 Hero Unified Orchestrator: Concurrency Status');
        console.log('='.repeat(80));
        ...
        console.log(`  Active: ${status.concurrencyStatus.buildOperations.activeOperations}`);
        console.log(`  Queued: ${status.concurrencyStatus.buildOperations.queuedOperations}`);
        console.log(`  Config: ${status.concurrencyStatus.buildOperations.config.maxConcurrent} max concurrent`);
        console.log('');
        console.log('Test Operations:');
        console.log(`  Active: ${status.concurrencyStatus.testOperations.activeOperations}`);
        console.log(`  Queued: ${status.concurrencyStatus.testOperations.queuedOperations}`);
        console.log(`  Config: ${status.concurrencyStatus.testOperations.config.maxConcurrent} max concurrent`);
        console.log('');
